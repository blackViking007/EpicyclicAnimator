...
'''
    Implementation of 2-opt algorithm for travelling salesman problem.
    Taken from: https://stackoverflow.com/a/44080908
    Yeah, kulted this! :/
'''
...
# npts = 200
# basepts = np.random.uniform(0.0,1.0,npts)
# cities = np.asarray([np.cos(2*np.pi*basepts)+np.random.normal(0.0,0.1,npts),np.sin(2*np.pi*basepts)+np.random.normal(0.0,0.1,npts)])
# print cities.shape
# cities = (cities-np.min(cities))/(np.max(cities)-np.min(cities))
# cities = np.transpose(cities,[1,0])
